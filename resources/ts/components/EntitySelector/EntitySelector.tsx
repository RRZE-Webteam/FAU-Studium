 ...
 * @ignore
 *
 * @param {?ReactElement}            label
 * @param {Object}                   messages
 * @param {?number}                  maxLength
 * @param {function(Array): void}    onChange
 * @param {function(object): string} entityToToken
 * @param {number}                   maxSuggestions
 * @param {Array}                    entities
 * @param {Array}                    searchedEntities
 * @param {function(string): void}   setSearch
 * @param {?boolean}                 childOnly
 *
 * @see https://github.com/WordPress/gutenberg/blob/c02421f8cd6e90966b7e8cd487d311ee31a6aada/packages/editor/src/components/post-taxonomies/flat-term-selector.js
 */
export default function EntitySelector< Entity extends GenericEntity >( {
	...
	const suggestions = useMemo< Array< string > >( () => {
		...
		const topLevelEntities = searchedEntities.filter(
	...
	const onChangeTokens = ( tokens ) => {
		...
		const newEntities = uniqueTokens.map( ( token ) =>
			find(
				availableEntities,
				( entity ) => entityToToken( entity ) === token
			)
		);
	...
	useEffect( () => {
		...
		const suggestionsList =
			wrapperRef.current.querySelector< HTMLUListElement >(
