import { useFacultyTerms } from './useTaxonomyTerm';
...
const ALLOWED_FACULTY_SLUGS_FOR_COMBINATION = [
	FACULTY_PHILOSOPHY,
	FACULTY_NATURAL_SCIENCES,
];
...
export function useDegreeFeesEnabled() {
	const [ feeRequired ] =
		useDegreeProgramProperty< boolean >( 'fee_required' );
...
export function useAdmissionRequirementsTeachingDegreeAtHigherSemesterEnabled() {
	...
	const [ bachelorOrTeachingAdmissionRequirement ] =
		useDegreeProgramProperty< AdmissionRequirement >(
	...
	return (
		...
		bachelorOrTeachingAdmissionRequirement.name.de !==
			ADMISSION_REQUIREMENT_FREE &&
		bachelorOrTeachingAdmissionRequirement?.parent?.name?.de !==
			ADMISSION_REQUIREMENT_FREE
	);
}
...
export function useLanguageSkillsForFacultyOfHumanitiesOnlyEnabled() {
	...
	return (
		...
		!! facultyTerms.find(
			( facultyItem ) => facultyItem.slug === FACULTY_PHILOSOPHY
		)
	);
}
...
function useSemestersContain( name: string ): boolean {
	const [ semesters ] =
		useDegreeProgramProperty< MultilingualString[] >( 'start' );
