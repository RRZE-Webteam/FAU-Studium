...
function systemRequirements() {
    ...
    if (version_compare(PHP_VERSION, RRZE_PHP_VERSION, '<')) {
        ...
        $error = sprintf(__('The server is running PHP version %1$s. The Plugin requires at least PHP version %2$s.', 'fau-studium'), PHP_VERSION, RRZE_PHP_VERSION);
    } elseif (version_compare($GLOBALS['wp_version'], RRZE_WP_VERSION, '<')) {
        /* Übersetzer: 1: aktuelle WP-Version, 2: erforderliche WP-Version */
        $error = sprintf(__('The server is running WordPress version %1$s. The Plugin requires at least WordPress version %2$s.', 'fau-studium'), $GLOBALS['wp_version'], RRZE_WP_VERSION);
    }
    return $error;
}
    ...
    if ($error = systemRequirements()) {
        add_action('admin_init', function () use ($error) {
            ...
            add_action($tag, function () use ($pluginName, $error) {
                printf(
                    '<div class="notice notice-error"><p>' . __('Plugins: %1$s: %2$s', 'fau-studium') . '</p></div>',
                    esc_html($pluginName),
                    esc_html($error)
                );
            });
